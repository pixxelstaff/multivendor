...
<head>
    ...
    $orders_qry = mysqli_prepare($con, "SELECT * FROM `order_items` WHERE  `admin_trash` = ?");
    mysqli_stmt_bind_param($orders_qry, 's', $trash_status);
                    ...
                    <div class="col-12  p-2 justify-content-start align-items-end gap-2 d-flex mt-4 other-type-filter">
                        <div class="filter-inp-div">
                            ...
                            <select name="action_select" class="product-sort w-100" id='act-select'>
                                ...
                                <option value="-1">Restore items</option>
                            </select>
                    ...
                    <div class="col-12 table-responsive product-wrapper">
                        <table class=" product-table w-100">
                            ...
                            <tbody>
                                <?php
                                $sno = 0 + $offset;
                                $order__data = mysqli_prepare($con, "SELECT * FROM `order_items` WHERE  `admin_trash` = ? ORDER BY `Id` DESC LIMIT $limit OFFSET $offset");
                                ...
                                ?>
                                    <tr>
                                        <td><input type="checkbox" class="mini-checkbox" name="item[]" value="<?= $sh['Id'] ?>"></td>
                                        <td><?= $sno  ?></td>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['sub_data'])) {
        $error = '';
        $restore = '';
        if (!empty($_POST['action_select']) && !empty($_POST['item'])) {
            $order__ids = $_POST['item'];
            if (is_array($order__ids)) {
                ...
                foreach ($order__ids as $sng__order_id) {
                    ...
                    if (mysqli_num_rows($orderData) > 0) {
                        ...
                        if ($order__sts == '1') {
                            ...
                            mysqli_stmt_bind_param($update_order_itms, 'ss', $restore, $sng__order_id);
                            if (mysqli_stmt_execute($update_order_itms)) {
                                $suc__msg .= 'successfully restore the items';
                            } else {
                                $suc__msg = '';
                ...
                alerting('something went wrong', 'item-trash.php');
            }
        } else {
            alerting('please select at least one item', 'item-trash.php?page=' . $page);
        }
    } else {
        alerting('something went wrong', 'item-trash.php?page=' . $page);
    }
}

if (!empty($suc__msg)) {
    alerting($suc__msg, 'item-trash.php?page=' . $page);
} elseif (!empty($error)) {
    alerting($error, 'item-trash.php?page=' . $page);
}
