    ...
    if (isset($_POST['sub_data'])) {
        ...
        if (!empty($_POST['action_select']) && !empty($_POST['orderId'])) {
            ...
            if (is_array($order__ids)) {
                        ...
                        if ($order__sts == '1') {
                            ...
                            if (count($unqStatus) == 1 && in_array('1', $unqStatus)) {
                                if ($_POST['action_select'] == '1') {
                                    ...
                                    mysqli_stmt_bind_param($trash_order, 'ss', $trash_back, $sng__order_id);
                                    if (mysqli_stmt_execute($trash_order)) {
                                        ...
                                        mysqli_stmt_bind_param($update_order_itms, 'ss', $trash_back, $sng__order_id);
                                        if (mysqli_stmt_execute($update_order_itms)) {
                                            $suc__msg = 'successfully restore the orders and it items';
                                        } else {
                                            $suc__msg = '';
                                    ...
                                    $delete_order = mysqli_prepare($con, "DELETE FROM `order` WHERE `Id` = ? ");
                                    mysqli_stmt_bind_param($delete_order, 's', $sng__order_id);
                                    if (mysqli_stmt_execute($delete_order)) {
                                        $suc__msg = 'successfully delete the orders';
                                        $delete_order_items = mysqli_prepare($con, "DELETE FROM `order_items`  WHERE `order_id` = ? ");
                                        mysqli_stmt_bind_param($delete_order_items, 's', $sng__order_id);
                                        if (mysqli_stmt_execute($delete_order_items)) {
                                            $suc__msg = 'successfully delete the orders and it items';
                                        } else {
                                            $suc__msg = '';
                                            $error = 'something went wrong for deleting order items';
                                        }
                                    };
                                }
                            } else {
                ...
                alerting('something went wrong', 'order-trash.php?page=' . $page);
            }
        } else {
            alerting('something is empty', 'order-trash.php?page=' . $page);
        }
    } else {
        alerting('something went wrong', 'order-trash.php?page=' . $page);
    }
...
if (!empty($suc__msg)) {
    alerting($suc__msg, 'order-trash.php?page=' . $page);
} elseif (!empty($error)) {
    alerting($error, 'order-trash.php?page=' . $page);
}
