...
<body class="g-sidenav-show   bg-gray-100">
    ...
    <div class="success-wrapper">
        <div class="success-message-container d-none">
            ...
            <a href="product.php" class="close-btn" id="pop-close-btn">
                <span class="material-symbols-outlined">
                    close
                </span>
            </a>
...
if (isset($_POST['sub']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($title) && !empty($sanitize_description) && !empty($price) && !empty($category)) {
        ...
        if ($price > $sale_price) {
            ...
            if (count($_FILES['m_image']['name']) > 0 && $_FILES['m_image']['error'] === UPLOAD_ERR_OK) {
            ...
            $productQ = "UPDATE `product` SET `name` = ?, `description` = ?, `price` = ?, `sale_price` = ?, `quantity` = ?, `category` = ?, `sku` = ?, `vendor_id` = ?, `product_type` = ?, `featured_image` = ?, `gallery_image` = ?, `tags` = ?, `dimensions` = ?, `excerpt` = ?,`stock` =?  ,`date` = ? WHERE `id` = ?";
            ...
            // Bind parameters
            mysqli_stmt_bind_param($productQ__p, "sssssssssssssssss", $title, $sanitize_description, $price, $sale_price, $quantity, $category, $sku, $vendor_id, $p_type, $img, $galImg, $tags, $dimension, $exercept, $stock, $date, $id);
