...
<head>
    ...
    // $orders = fetchOtherdetails($con, 'order_items', 'item_type', 'product');
    $itemType = 'product';
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr class="<?= $sh['admin_view'] == '0' ? 'un-viewed' : '' ?>">
                                        <td><input type="checkbox" class="mini-checkbox" name="productitem[]" value="<?= $sh['Id'] ?>"></td>
                                        <td><?= $sno  ?></td>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['sub_data'])) {
        $error = '';
        if (!empty($_POST['action_select']) && !empty($_POST['productitem'])) {
            $order__ids = $_POST['productitem'];
            ...
            alerting('please select at least one product', 'product-orders.php?page=' . $page);
        }
    } else {
        alerting('something went wrong', 'product-orders.php?page=' . $page);
    }
}

if (!empty($suc__msg)) {
    alerting($suc__msg, 'product-orders.php?page=' . $page);
} elseif (!empty($error)) {
    alerting($error, 'product-orders.php?page=' . $page);
}
