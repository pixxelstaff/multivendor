...
<head>
    ...
    if (isset($_GET['id'])) {
        ...
        if(mysqli_num_rows($data) > 0){
            ...
            alerting("the updating service has been removed or doensot exist with respect to your current data ",'service.php');
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sub'])) {
    if (!empty($_POST['title']) && !empty($_POST['service_description']) && !empty($_POST['regular_price']) && !empty($_POST['service']) && $_POST['service-category'] != '') {
        ...
        if ($price > $sale_price) {
            ...
            $qry = "UPDATE `service` SET `name`= ?,`description`=?,`price`=?,`sale_price`=?,`service_type`=?,`available`=?,`start_date`=?,`end_date`=?,`start_time`=?,`end_time`=?,`slot`=?,`city`=?,`area`=?,`category`=?,`location`=?,`services_data`=?,`excerpt`=?,`featured_image`=?,`gallery_images`=?,`video` = ?,`tags`=?,`validity`=?,`date`=? WHERE `Id` = ? ";

            $prepare_qry = mysqli_prepare($con, $qry);
            mysqli_stmt_bind_param($prepare_qry, 'ssssssssssssssssssssssss', $upd_title, $upd_sanitize_description, $upd_price, $upd_sale_price, $upd_type, $upd_available, $upd_start_date, $upd_end_date, $upd_start_time, $upd_end_time, $upd_slot, $upd_city, $upd_area, $upd_category, $location, $upd_service, $upd_excerpt, $img, $galImg, $video_name, $upd_tags, $upd_validity, $date, $id);
