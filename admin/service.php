...
<body class="g-sidenav-show   bg-gray-100">
    ...
    <main class="main-content position-relative border-radius-lg ">
        ...
        $count_qry  = mysqli_prepare($con,"SELECT * FROM `service` WHERE `status` = ? AND `trash` != ?");
        mysqli_stmt_bind_param($count_qry,'ss',$appr,$trash);
        if(mysqli_stmt_execute($count_qry)){
            $result_count = mysqli_stmt_get_result($count_qry);
        }
        ...
        <div class="container-fluid p-4">
            ...
            <div class="col-12 p-2 rounded bg-white">
                ...
                mysqli_stmt_bind_param($showservice__prepare,'ss',$appr,$trash);
                                        ...
                                        <tr class="<?= $sh['viewed'] == '0' ? 'un-viewed' : '' ?>">
                                            ...
                                            <td>
                                                <div class="action-button-div">
                                                    ...
                                                    if ($sh['user_id'] == '0') {
                                                    ?>
                                                        <a href="update-service.php?id=<?= base64_encode($sh['Id']) ?>" class="edit-btn edit-pop">
                                                            <span class="material-symbols-outlined">
                                                                edit
                                                            </span>
                                                        </a>
                                                        <a href="?page=<?= $page ?>&del_id=<?= base64_encode($sh['Id']) ?>" class="delete-btn">
                                                            <span class="material-symbols-outlined">
                                                                delete_forever
                                                            </span>
                                                        </a>
                                ...
                                <div class="col-lg-3 col-md-6 col-sm-12 product-div">
                                    <div class="col-12 product-inner-div">
                                        <div class="product-images-div">
                                            ...
                                            <?php
                                            $galleryimg = explode(',', $sh['gallery_images']);
                ...
                ?>
                    <script>
                        window.location.href = 'service.php?page=<?= $totalPage  ?>'
                    </script>
                <?php
                }
                $startNo = 1 + $offset;
            ...
            if (is_array($service_ids)) {
                ...
                foreach ($service_ids as $sng__service_id) {
                    ...
                    if (mysqli_num_rows($serviceData) > 0) {
                            ...
                            mysqli_stmt_bind_param($trash_order, 'ss', $trash, $sng__service_id);
                            if (mysqli_stmt_execute($trash_order)) {
                                $suc__msg = 'successfully trash the service';
                            } else {
                                $suc__msg = '';
                ...
                alerting('something went wrong', 'service.php?page=' . $page . '&view=' . $view);
            }
        } else {
            alerting('please select at least one service', 'service.php?page=' . $page . '&view=' . $view);
        }
    } else {
        alerting('something went wrong', 'service.php?page=' . $page . '&view=' . $view);
    }
...
if (!empty($suc__msg)) {
    alerting($suc__msg, 'service.php?page=' . $page . '&view=' . $view);
} elseif (!empty($error)) {
    alerting($error, 'service.php?page=' . $page . '&view=' . $view);
}
