    ...
    if (isset($_POST['sub_data'])) {
        ...
        if (!empty($_POST['action_select']) && !empty($_POST['vendorId'])) {
            ...
            if (is_array($vendor__ids)) {
                ...
                alerting('something went wrong', 'terminated.php?page='.$page);
            }
        } else {
            alerting('please select atleast one', 'terminated.php?page='.$page);
        }
    } else {
        alerting('something went wrong', 'terminated.php?page='.$page);
    }
...
if (!empty($suc__msg)) {
    alerting($suc__msg, 'terminated.php?page='.$page);
} elseif (!empty($trash_error)) {
    alerting($trash_error, 'terminated.php?page='.$page);
}
