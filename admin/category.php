...
<body class="g-sidenav-show   bg-gray-100">
            ...
            <div class="col-12 p-2 rounded bg-white">
                ...
                <div class="col-12 table-responsive product-wrapper ">
                    ...
                    <table class=" product-table w-100">
                        ...
                        <tbody>
                            <?php
                            $count_category = fetchAllData($con, 'category');
                            ...
                            $s_no = 0;
                            $list_q = "SELECT * FROM `category` ORDER BY `Id` DESC LIMIT $limit OFFSET $offset";
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            $name = fetchOtherdetails($con, 'category', 'Id', $sh['parentId']);
                                        ...
                                        <td>
                                            <div class="action-button-div">
                                                <a href="javascript:void(0)" class="eye-btn eye-pop" data-category-page='product' data-action-id="<?= $sh['Id'] ?>">
                                                    <span class="material-symbols-outlined ">
                                                        visibility
                                                    </span>
                                                </a>
                                                <a href="javascript:void(0)" class="edit-btn edit-pop" data-category-page='product' data-action-id="<?= $sh['Id'] ?>">
                                                    <span class="material-symbols-outlined">
                                                        edit
                                                    </span>
                                                </a>
    ...
    <div class="success-wrapper">
        <div class="success-message-container d-none">
            ...
            <p class="pop-message">the category is uploaded successfully.now u can upload products with using these categroy </p>
...
if (isset($_POST['i_sub']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST['name']) && $_POST['p_cat'] != '') {
        ...
        if (empty($error_message)) {
            ...
            $selQ = fetchOtherdetails($con, 'category', 'name', $name);
            if (mysqli_num_rows($selQ) == 0) {
                $insert_Q = "INSERT INTO `category`(`name`,`parentId`,`description`,`image`,`user`,`date`) VALUES(?,?,?,?,?,?)";
...
if (isset($_POST['upd_submit']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name) && $category != '') {
        ...
        $updateQ = "UPDATE `category` SET `name`  = ? , `parentId` = ? , `description` = ? , `image` = ? , `user` = ? ,`date` = ? WHERE `Id` = ?";
...
if (isset($_GET['delete-id'])) {
    ...
    $deleteQ = mysqli_prepare($con, "DELETE FROM `category` WHERE `Id` =?");
