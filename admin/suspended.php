...
<head>
    ...
    $limit = 10;
    $offset = ($page - 1) * $limit;
    $qry = mysqli_query($con, "SELECT * FROM `vendor` WHERE `approved` = '-2' AND `trash` != '-1'");
                                ...
                                ?>
                                            <tr class="<?= $sh['viewed'] == '0' ? 'un-viewed' : '' ?>">
                                                ...
                                                <td> <select name="" data-current-state=<?= $status ?> data-v-name=<?= dec_function($sh['name']); ?> data-vendor-id='<?= $sh['Id'] ?>' class="product-sort w-120 items_state_pointer account_status">
                                                        ...
                                                        <option value="1">Approve back </option>
                                                ...
                                                <td>
                                                    <div class="action-button-div">
                                                        ...
                                                        <a href="?delete-vendor=<?= base64_encode($sh['Id']) ?>" class="delete-btn">
                                                            <span class="material-symbols-outlined">
                                                                delete_forever
                                                            </span>
                                                        </a>
    ...
    if (isset($_POST['sub_data'])) {
        ...
        if (!empty($_POST['action_select']) && !empty($_POST['vendorId'])) {
            ...
            if (is_array($vendor__ids)) {
                ...
                alerting('something went wrong', 'suspended.php?page='.$page);
            }
        } else {
            alerting('please select atleast one', 'suspended.php?page='.$page);
        }
    } else {
        alerting('something went wrong', 'suspended.php?page='.$page);
    }
...
if (!empty($suc__msg)) {
    alerting($suc__msg, 'suspended.php?page='.$page);
} elseif (!empty($trash_error)) {
    alerting($trash_error, 'suspended.php?page='.$page);
}
